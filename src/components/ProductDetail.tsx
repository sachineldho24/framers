    ...
    <main className="pb-36 pt-16">
      ...
      <section className="mx-auto max-w-4xl space-y-12 px-margin-mobile py-12">
            ...
            {siblings.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`border-2 border-border-high-contrast py-4 font-bold uppercase transition-all ${
                    active ? "bg-neon-accent text-black" : "hover:bg-surface-container hover:text-white"
                  }`}
                >
                  {s.name}
                </button>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          <div className="flex flex-col justify-between border-2 border-border-high-contrast bg-surface-muted p-6">
      ...
      {/* Sticky CTA */}
      <div className="fixed bottom-0 left-0 z-50 w-full border-t-2 border-border-high-contrast bg-surface/80 p-4 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl flex-col gap-2">
          <button
            onClick={chooseDesignMethod}
            disabled={loading}
            className="w-full border-2 border-border-high-contrast bg-action-red py-5 font-bold uppercase tracking-widest text-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-60"
          >
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-4">
      <div className="h-1 w-12 bg-outline" />
