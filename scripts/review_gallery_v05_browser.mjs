...
if(!ready)throw new Error('Viewer did not load in 120 seconds');
await new Promise(r=>setTimeout(r,500));
const shot=await call('Page.captureScreenshot',{format:'png'});
await fs.writeFile('C:/Personal_Projects/Framers/gallery_v05/previews/browser-tour-entry.png',Buffer.from(shot.data,'base64'));
...
for(const progress of [0,.2,.4,.55,.82,.955,1]){
 ...
 await fs.writeFile(`C:/Personal_Projects/Framers/gallery_v05/previews/tour-${Math.round(progress*1000)}.png`,Buffer.from(sample.data,'base64'));
}
const replay=[];
...
await fs.writeFile('C:/Personal_Projects/Framers/gallery_v05/previews/browser-mobile.png',Buffer.from(mobileShot.data,'base64'));
await call('Emulation.clearDeviceMetricsOverride');
await fs.writeFile('C:/Personal_Projects/Framers/gallery_v05/browser-validation.json',JSON.stringify({initial:initial.result.value,idleFramesRendered:idleEnd-idleStart,stops:results,replay,playbackProgress:playback.result.value,orbitEnabled:orbit.result.value,mobile:mobile.result.value,errors},null,2));
