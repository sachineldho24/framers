...
if(!resume){
 ...
 await call('Page.navigate',{url:'http://127.0.0.1:8766/?review='+Date.now()});
}
...
if(!ready)throw new Error('Viewer did not load in 240 seconds');
await new Promise(r=>setTimeout(r,500));
if(!resume){
 const shot=await call('Page.captureScreenshot',{format:'png'});
 await fs.writeFile('C:/Personal_Projects/Framers/gallery_v06/previews/browser-tour-entry.png',Buffer.from(shot.data,'base64'));
}
const initial=await call('Runtime.evaluate',{expression:'({revision:gallery.revision,meshyMeshes:(()=>{let count=0;gallery.gltf.scene.traverse(o=>{if(o.isMesh&&o.name.includes("MESHY"))count++});return count})(),duration:gallery.gltf.animations[0].duration,cameras:gallery.rooms,drawCalls:gallery.renderer.info.render.calls,triangles:gallery.renderer.info.render.triangles,textureCount:gallery.renderer.info.memory.textures})',returnByValue:true});
const idleStart=(await call('Runtime.evaluate',{expression:'gallery.renderer.info.render.frame',returnByValue:true})).result.value;
await new Promise(r=>setTimeout(r,650));
const idleEnd=(await call('Runtime.evaluate',{expression:'gallery.renderer.info.render.frame',returnByValue:true})).result.value;
if(idleEnd-idleStart>1)throw new Error('Idle viewer keeps rendering');
const results=[];
for(const progress of [0,.2,.4,.55,.82,.955,1]){
 ...
 if(!resume){
  ...
  await fs.writeFile(`C:/Personal_Projects/Framers/gallery_v06/previews/tour-${Math.round(progress*1000)}.png`,Buffer.from(sample.data,'base64'));
 }
}
const replay=[];
...
for(let attempt=0;attempt<60;attempt++){
 await new Promise(r=>setTimeout(r,500));
 playback=await call('Runtime.evaluate',{expression:'Number(document.querySelector("#progress").value)',returnByValue:true});
 if(playback.result.value>400)break;
}
await call('Runtime.evaluate',{expression:'document.querySelector("#play").click()'});
...
await fs.writeFile('C:/Personal_Projects/Framers/gallery_v06/previews/browser-mobile.png',Buffer.from(mobileShot.data,'base64'));
await call('Emulation.clearDeviceMetricsOverride');
await fs.writeFile('C:/Personal_Projects/Framers/gallery_v06/browser-validation.json',JSON.stringify({initial:initial.result.value,idleFramesRendered:idleEnd-idleStart,stops:results,replay,playbackProgress:playback.result.value,orbitEnabled:orbit.result.value,mobile:mobile.result.value,errors},null,2));
